...
namespace backend {
    ...
    bool getPhoneLocation(double& lat, double& lon, float& accuracy, bool& hasFix) {
        ...
        jmethodID has_fix_id = java_env->GetMethodID(native_activity_clazz, "hasGpsFix", "()Z");
        jmethodID lat_id = java_env->GetMethodID(native_activity_clazz, "getGpsLatitude", "()D");
        jmethodID lon_id = java_env->GetMethodID(native_activity_clazz, "getGpsLongitude", "()D");
        jmethodID acc_id = java_env->GetMethodID(native_activity_clazz, "getGpsAccuracy", "()F");
        if (!has_fix_id || !lat_id || !lon_id || !acc_id) {
            java_vm->DetachCurrentThread();
            return false;
        }

        hasFix = java_env->CallBooleanMethod(app->activity->clazz, has_fix_id);
        if (hasFix) {
            lat = java_env->CallDoubleMethod(app->activity->clazz, lat_id);
            lon = java_env->CallDoubleMethod(app->activity->clazz, lon_id);
            accuracy = java_env->CallFloatMethod(app->activity->clazz, acc_id);
        }
    ...
    bool openMapView() {
        ...
        jmethodID method_id = java_env->GetMethodID(native_activity_clazz, "openMapView", "()V");
        if (method_id == NULL) {
            java_vm->DetachCurrentThread();
            return false;
        }
